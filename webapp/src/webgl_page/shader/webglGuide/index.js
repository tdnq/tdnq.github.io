...
export default class WebglGuide extends Base {
  ...
  rotateCubeUseMouse(ele) {
    ...
    const u_viewMatrix = this.gl.getUniformLocation(this.gl._program, 'u_transformMatrix');
    // bind event
    this.rotateCubeUseMouse_bind(viewMatrix, u_viewMatrix);
  }

  rotateCubeUseMouse_model() {
    const vertex = new Float32Array(cube.vertex);
    const pointNum = vertex.length / cube.unit;
    const indices = new Uint8Array(cube.indices);
    const color = new Float32Array(this.getRandomArray(4 * pointNum));
  ...
  rotateCubeUseMouse_bind(viewMatrix, u_viewMatrix) {
    ...
    this.ele.addEventListener('mousemove', (event) => {
      if (isMove) {
        ...
        const y = event.clientY - elePosition.y;
