...
export default function bezierCurve(ele, data) {
  ...
  this.ele.addEventListener('click', (e) => {
    ...
    const x = ((e.clientX - elePosition.x) - (e.target.width / 2)) / (e.target.width / 2);
    const y = -((e.clientY - elePosition.y) - (e.target.height / 2)) / (e.target.height / 2);
    this.gl.clear(this.gl.DEPTH_BUFFER_BIT | this.gl.COLOR_BUFFER_BIT);
...
function model(points, data = { frequence: 0.01 }) {
  for (let t = 0; t < 1; t += data.frequence) {
    const bezierPoint = recursive_bezier(points, t);
    const vertex = new Float32Array([bezierPoint.x, bezierPoint.y, bezierPoint.z]);
    const pointNum = vertex.length / 3;

    const colorArr = this.getRandomArray(4 * pointNum);
    colorArr[3] = 1;
